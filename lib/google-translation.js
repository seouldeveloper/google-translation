...
client.translate = function(obj, callback){
  ...
  if (requestModel === "nmt") {
	  ...
	  if(source) {
		  url = 'https://www.googleapis.com/language/translate/v2?key=' + client.apiKey + '&q=' + encodeURI(text) + '&source=' + source + '&target=' + target;
	  } else {
		  url = 'https://www.googleapis.com/language/translate/v2?key=' + client.apiKey + '&q=' + encodeURI(text) + '&target=' + target;
	  }
	  url = 'https://www.googleapis.com/language/translate/v2?key=' + client.apiKey + '&q=' + encodeURI(text) + '&target=' + target;
	  options = {
		  url,
